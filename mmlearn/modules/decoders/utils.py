...
def extract_locations_from_token_ids(tokens, n=4):
  ...
  for token in tokens:
    ...
    # if (32000 <= token).any() & (token < 33000).any():
      box.append(token)
      if len(box) == n:
...
'''
def extract_locations_from_token_ids(tokens_batch, n=4):
    all_boxes = []
    ...
    for tokens in tokens_batch:
        ...
        for token in tokens:
            if 32000 <= token < 33000:
                ...
                # Reset the box if the token is out of the expected range
                box = []
...
def undo_box_preprocessing(boxes, image_info):
  ...
  # paddings = np.array([top_pad.cpu(), left_pad.cpu(), top_pad.cpu(), left_pad.cpu()], dtype=boxes.dtype)
  paddings = np.array([top_pad[0].item(), left_pad[0].item(), top_pad[0].item(), left_pad[0].item()], dtype=boxes.dtype)
  
  paddings = torch.tensor(paddings, device='cuda')
  paddings = paddings.unsqueeze(0).expand(boxes.shape[0], -1)
  
  # paddings = paddings.expand(, -1)

  if len(boxes.shape) == 1:
    boxes = boxes - paddings.cpu().numpy()
  else:
    boxes = boxes - np.expand_dims(paddings.cpu().numpy(), axis=1)
  ...
  off_y = image_info[:, 7].cpu().numpy().astype(int).tolist()
  off_x = image_info[:, 8].cpu().numpy().astype(int).tolist()
  
  # assert off_x == off_y == 0
  off_x = np.array(off_x) if not isinstance(off_x, np.ndarray) else off_x
  off_y = np.array(off_y) if not isinstance(off_y, np.ndarray) else off_y
  assert np.all(off_x == 0).item() and np.all(off_y == 0).item()

  # Undo the scaling
  inv_scale = image_info[:, 2]
  boxes = boxes * inv_scale.cpu().numpy().reshape(-1, 1, 1)

  # clip in case the model predicted a region in the padded area
  h, w = image_info[:, 3], image_info[:, 4]
  ...
  boxes = np.minimum(boxes, np.expand_dims(np.array([h, w, h, w]).T, axis=1))
...
def undo_box_preprocessing(boxes, image_info):
  ...
  # Undo the scaling
  inv_scale = image_info[2]
  boxes = boxes * inv_scale#.cpu().numpy()
