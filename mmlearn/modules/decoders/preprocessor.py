...
from mmlearn.modules.decoders import config
from mmlearn.modules.decoders.audio_utils import load_audio
from mmlearn.modules.decoders.config import get_tokenizer, Config
from mmlearn.modules.decoders.data_utils import resize_and_pad_default, values_to_tokens
from mmlearn.modules.decoders.get_modality_processor import get_input_modalities, get_target_modalities
from mmlearn.modules.decoders.utils import flatten_dict
from mmlearn.modules.decoders.video_utils import load_video, remove_bars_from_frames
...
class UnifiedIOPreprocessor(FeatureExtractionMixin):
  ...
  def __call__(
    ...
    if box_inputs:
      ...
      # box_text = " ".join([x.decode("utf-8") for x in box_text.numpy()[0][0]])
...
def build_batch(examples: List[Dict[str, np.ndarray]], device=config.DEVICE) -> Dict[str, np.ndarray]:
  ...
  for key in keys:
    ...
    # logging.info(f"========== vals: {vals} ---- type {type(vals[0])}")
    max_len = max(len(v) if v is not None else 0 for v in vals)
    out = np.zeros([len(examples), max_len]+sh, dtype=val.dtype)
    if key == "caption" or key == "labels":
      # TODO maybe uncomment for image captioning?
      # max_len = max(len(v[0]) if v is not None else 0 for v in vals)
      out = np.zeros([len(examples), max_len]+sh, dtype=object)
      # out = np.zeros([len(examples), max_len]+sh, dtype=val.dtype)
  ...
  for k, v in out_dict.items():
      if k != "labels" and k != "caption":
          out_dict[k] = torch.as_tensor(v, device=device)
      else:
          # Add your else logic here
          out_dict[k] = v  # Example: Just keep the original value
