...
class SentencePieceVocabulary:
  ...
  def _decode(self, ids):
    ...
    ids = torch.where(ids >= piece_size, self.unk_id, ids)
    return self.tokenizer.DecodeIds(ids.tolist())
  ...
  def decode(self, ids: Iterable[int]):
    ...
    # clean_ids = list(ids)
    clean_ids = torch.tensor(ids, dtype=torch.int32)

    if self.unk_id is not None:
      vocab_size = self._base_vocab_size
      # clean_ids = [self.unk_id if i >= vocab_size else i for i in clean_ids]
      clean_ids = torch.where(clean_ids >= vocab_size, self.unk_id, clean_ids)
      
    clean_ids = clean_ids.tolist()
