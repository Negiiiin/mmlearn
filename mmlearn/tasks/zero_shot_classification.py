...
class ZeroShotClassification(EvaluationHooks):
    ...
    tokenizer :
        A tokenizer to tokenize the text descriptions of labels.
    """
    ...
    def on_evaluation_epoch_start(
        ...
        def _process_batch(batch_classnames):
            ...
            texts = [template.format(c) for c in batch classnames for template in templates]
            tokens = self.tokenizer(texts).to(pl_module.device)
            
            class_embeddings = pl_module.encode_text(texts, normalize=True)
